...
$countSql = "SELECT COUNT(*) FROM questions q WHERE 1=1";
$countParams = [];

// Re-apply filters for count
if ($category) {
    $countSql .= " AND q.category = ?";
...
if ($status) {
    if ($status === 'active') {
        $countSql .= " AND q.is_active = 1";
    } elseif ($status === 'inactive') {
        $countSql .= " AND q.is_active = 0";
    }
}
if ($ai_generated !== null) {
    $countSql .= " AND q.ai_generated = ?";
    $countParams[] = $ai_generated === 'true' ? 1 : 0;
}
if ($search) {
    $countSql .= " AND q.question_text LIKE ?";
    $countParams[] = "%$search%";
}
...
jsonResponse(['data' => $questions, 'total' => $total]);
?>
