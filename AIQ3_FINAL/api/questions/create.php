...
// Let's check for duplicates based on Type and Correct Answer for Image questions.
if (in_array($type, ['image_identify_logo', 'image_identify_person'])) {
    $stmt = $pdo->prepare("SELECT id FROM questions WHERE question_type = ? AND correct_answer = ?");
    $stmt->execute([$type, $correctAnswer]);
    if ($stmt->fetch()) {
        jsonResponse(['error' => 'A question of this type with this answer already exists.'], 409);
    }
} else {
    // For text questions, check text uniqueness
    $stmt = $pdo->prepare("SELECT id FROM questions WHERE question_text = ?");
    $stmt->execute([$questionText]);
    if ($stmt->fetch()) {
        jsonResponse(['error' => 'A question with this text already exists.'], 409);
    }
}
