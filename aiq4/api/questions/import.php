...
try {
    ...
    foreach ($questions as $index => $question) {
        try {
            ...
            // Stricter duplicate check: Question is only a duplicate if Text, Type, AND Correct Answer match.
            // This allows generic prompts like "Identify this logo" to be used for different logos.
            $stmt = $pdo->prepare("
                SELECT id, is_active 
                FROM questions 
                ...
                LIMIT 1
            ");
            $stmt->execute([$questionText, $questionType, $correctAnswer]);
            $existing = $stmt->fetch();
            ...
            // For general text questions, we still want to avoid very similar text if type and answer are same
            if (!str_contains($questionType, 'image_identify')) {
                ...
                $stmt = $pdo->prepare("
                    ...
                    AND question_type = ?
                    AND LOWER(TRIM(correct_answer)) = ?
                    LIMIT 1
                ");
                $stmt->execute([$normalizedText, $questionType, strtolower(trim($correctAnswer))]);
                $similar = $stmt->fetch();
